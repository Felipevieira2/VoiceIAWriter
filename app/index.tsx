...
export default function Home() {
  ...
  const visualizerScale = 1 + audioLevel * 1.5;
    ...
    <SafeAreaView className="flex-1 bg-slate-950">
      <View className="flex-1 flex flex-col items-center relative px-4">
        ...
        <View className="flex-row items-center justify-between w-full pt-4 pb-2 z-10">
          ...
          <TouchableOpacity className="items-center justify-center w-10 h-10 rounded-full bg-white/5">
        ...
        <View className="flex-1 w-full items-center justify-center mb-20">
          ...
          {/* Visualizer */}
          <View className="w-full h-40 mb-10 items-center justify-center relative">
             <View 
                className={`absolute w-40 h-40 bg-blue-500/20 rounded-full blur-3xl ${isRecording && !isPaused ? 'opacity-100' : 'opacity-30'}`}
                style={{ transform: [{ scale: isRecording ? visualizerScale : 1 }], pointerEvents: 'none' }}
             />
             {/* Simple bar visualizer */}
             <View className="flex-row gap-1 h-20 items-center">
                {[...Array(20)].map((_, i) => (
                    <View 
                        key={i}
                        className="w-1 bg-blue-500 rounded-full"
                        style={{ 
                            height: isRecording && !isPaused ? Math.max(4, Math.random() * 80 * audioLevel + 10) : 4,
